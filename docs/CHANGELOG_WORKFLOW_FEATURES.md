...
Text-only MiniMax-H3 T2V/Playground examples can use the community INT8 ConvRot uncensored text encoder as a drop-in `CLIPLoader` replacement. Reference-to-video and image-to-video examples retain the stock encoder because the community encoder's visual path requires a dequantization backend not available in the supported ComfyUI setup. Matching the diffusion-model quantization does not fix that visual-path limitation; a compatible re-quantized text encoder would be required. This changes the text-encoder component only; it does not remove all model, provider, or workflow-level safety behavior.
